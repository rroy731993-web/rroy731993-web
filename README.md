### Hi, I'm Ravi 👋

**Manager – Business Process & Systems** at Birla Corporation Ltd (RCCPL), Maihar
12+ years turning manufacturing operations data into measurable results — turnaround time, cost efficiency, dispatch reliability.

- 🏭 Currently: SME for dispatch workflows, inventory reconciliation & compliance at a cement packing plant
- 📊 Background: Business analysis, SQL/Tableau dashboards, SAP PM, process digitization
- 🚀 Recent wins: cut rake loading TAT from 9→6.5 hrs, reduced specific power consumption 2.8→1.8 kWh/ton, 25% cut in slow-moving inventory
- 🎓 Currently studying: Business Analytics with Gen & Agentic AI (BITSOM)
- 🌱 Learning to apply GenAI/Agentic AI tools to real plant operations
- 📫 Reach me: rroy.731993@gmail.com

---

### 🛠️ Skills

**Business Analysis:** BRD/FRD · Stakeholder Interviews · Gap Analysis · UAT · Workflow Design
**Data & BI:** SQL · Python · Tableau · Excel/MIS · KPI Dashboards
**Systems & Ops:** SAP PM · Dispatch Workflows · Inventory Reconciliation · DCS (ABB 800xA)
**Continuous Improvement:** Six Sigma · TPM · Root-Cause Analysis · TAT Reduction
**Digital Transformation:** GenAI/Agentic AI · Power Automate · QR-based Inspections · ILMS Integration

---

### 📌 Featured Projects

**One Business Unit, One Inventory** — Unified warehouse strategy across SAP PM/MM that cut slow/non-moving inventory by 25%

**Digital Inspection & Dispatch (QR Inspection)** — Replaced paper checklists with a mobile QR-based system using Power Automate, SQL & Python — zero paper, lower equipment downtime

**Automated Bag Counting & Dispatch Digitization** — Automated bag-counting integrated with ILMS to eliminate manual dispatch data capture

---
### 💼 Experience Timeline
2025 – Present   Manager, Business Process & Systems       Birla Corporation Ltd (RCCPL), Maihar
2017 – 2025      Deputy Manager, Business Analysis & Ops    Nuvoco Vistas Corp Ltd, Jamshedpur
2017 (Feb–Oct)   Engineer III, Systems & Process Integration FLSmidth Pvt. Ltd, Gurgaon
2014 – 2017      Engineer, Instrumentation & Systems         Meghalaya Cements Limited

### 🎓 Education
MBA, Operations & Industrial Management — IIBMS, Mumbai (2018–2020) — A+

B.E., Electronics & Telecommunication Engineering — Padre Conceicao College of Engineering, Goa University (2010–2014) — Distinction

Business Analytics with Gen & Agentic AI — BITSOM (2025–2026, in progress)

